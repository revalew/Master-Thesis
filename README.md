...
Added UDP communication protocol to dramatically improve sensor data collection performance, when compared to HTTP: from $`[1, 5] Hz \qquad \to \qquad \gg 100 Hz`$ sampling rates (over 25x faster!!). Some packets may be lost in transmission, but I'm willing to live with that.
